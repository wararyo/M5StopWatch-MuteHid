...
参考実装はInputを`Data,Var,Abs`とし、ローカルのミュート値を送る。ただし本アプリでは、これを「会議アプリへの絶対状態設定命令」と確定しない。ホストが状態値として処理するか、押下／解放のエッジをトグルとして扱うかをPoCで観測する。

状態値方式と、必要に応じた押下→解放方式を別の実験構成として検証する。最終版には確認済みの方式だけを採用し、無反応を理由に複数方式を連続送信しない。押下／解放方式では解放送信の失敗も管理し、接続が続く場合は解放を完了させる。
...
| 既知状態で切り替え操作 | 確定済みの送信方式で1操作を送信し、「反映待ち」にする |
| 操作後のOutput受信 | 現在のホスト報告を優先する。期待と異なれば「PC側状態を反映」とし、成功と断定しない |
| 反映待ちが1秒超過 | `Unknown`へ移行し「状態を確認できません」。自動トグル再送しない |
...
`Unknown`では、トグルの意味が確認済みの構成に限り「状態不明のまま切替要求」を1回送れる。絶対値を必要とする構成では現在値を反転できないため操作を無効にする。この差は接続プロファイルで定義し、推測でミュート値を生成しない。
...
KantanPlayと同じく、独立ファームウェアをOTAスロットに置き、再起動で切り替える。UserDemoへのLVGLアプリ組み込みは行わない。現行のUserDemoとKantanPlayの共存テーブルは一致している。
...
| ota_1 | `0x510000` | `0x4F0000` | MuteHid |
| storage | `0xA00000` | `0x400000` | UserDemoの画像など |
| coredump | `0xE00000` | `0x010000` | 共有 |

この構成は **UserDemo＋MuteHidの2本共存**。KantanPlayが既にota_1にあれば、その置き換えになる。UserDemo＋KantanPlay＋MuteHidの3本共存は追加スロットと配置変更が必要なため別仕様とする。

- UserDemo側ランチャーは、ota_1のイメージ情報を確認し、MuteHidとして起動できる項目を追加または既存App1を汎用化する。別アプリをMuteHidと誤表示しない。
...
- 共存用ビルドと単独用ビルドを分け、共存版の通常uploadをガードする。更新は実機パーティション照合後にota_1だけを書き換える。
...
| 切断・PCスリープ・再起動後の再接続10回 | 古い操作を再生せず、初回通知まで不明表示 |
...
5. BLEボンドを含む共有NVS容量と、UserDemo側の保存領域との衝突有無。
